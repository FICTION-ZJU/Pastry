...
def array_from_string(s: str):
    """
    Converts a string to a sympy array.

    Args:
        s (str): A string of the form "(x,y,...)"

    Returns:
        The corresponding sympy array
    """
...
class SimpleProgram:
    ...
    def __init__(self, updates, dterm, guardvector, guardvalue):
        # Make updates scalar
        tmp_updates = [(e.to_scalar(guardvector), e.get_probability())
                       for e in updates]
    ...
    def get_characteristic_poly(self) -> sympy.Poly:
        """
        Calculates the characteristic polynomial of the program.
...
parser = argparse.ArgumentParser(
    description='Calculate exact runtime from file')
...
if args.directtermination is not None:
    direct_flag = True
...
if roots:
    ...
    A = matrix([[sympy.re(monom.subs(sympy.symbols('x'), -i))
                 for monom in r_monoms] for i in range(program.k)])
...
v_vars = sympy.Array(
    [sympy.symbols('x{id}'.format(id=i+1)) for i in range(vec_length)])
v = array_dot_product(v_vars, guardvec)-guardval
r = r.subs(sympy.symbols('x'), v)

# Quick and dirty version to fix the power operator
str_r = str(r).replace("**", "^")
print("STRING\n{res}".format(res=str_r))
